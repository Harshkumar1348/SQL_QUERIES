...
provider_name,
DATE_TRUNC('WEEK',LAST_DELIVERY_DATE) AS ldd_week,
city,
avg_rating,
...
avg_rating_l15 AS (
    SELECT
        provider_id,
        -- Removed WEEK grouping to get lifetime L15 per provider
        ROUND(SUM(rating) / NULLIF(COUNT(rating), 0), 2) AS avg_rating_l15
    FROM ranked_ratings
...
left join avg_rating_l15 arl on f.provider_id = arl.provider_id
),
...
count(distinct case when life_cycle='ELC' and churn_status='churn' and ldd_week=perf_week then provider_id end) as churn_pros,
...
round(sum(case when life_cycle='ELC' then total_deliveries end)/nullif(hh_eligible,0),2) as util,
round(sum(case when life_cycle='ELC' then avg_rating_l15 end ) / NULLIF(HH_ELIGIBLE,0),2) AS avg_l15_rating,
ROUND((churn_pros/hh_eligible),2)*100 as churn_perc,
ROUND(
    100.0 * 

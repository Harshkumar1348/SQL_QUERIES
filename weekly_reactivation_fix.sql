...
-- UPDATED reactivation logic:
-- 1. This week = Working (from markings > 8h)
-- 2. Last week = Not Working
-- 3. Provider has AT LEAST ONE churn event in the past (any time before this week)
reactivation AS (
    SELECT
        bf.reporting_city,
    ...
    WHERE bf.week_mark_status = 'Working'
      ...
      AND EXISTS (
          ...
          WHERE ce.provider_id = bf.provider_id
            AND ce.churn_week < bf.week_start  -- Changed from = (week - 1 week) to < week
      )
    GROUP BY 1,2
)

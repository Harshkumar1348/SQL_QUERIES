...
ls AS (
    SELECT
        DATE_TRUNC('WEEK', m.BDATE) AS week_start,
        m.reporting_city AS city,
        m.provider_id,
        COUNT(
            CASE
                WHEN TIMESTAMPDIFF(
        ...
        COUNT(DISTINCT CASE 
            ...
            THEN m.customer_request_id 
        END) AS late_customer_ids
    FROM MASTER_DATA_EXPLORE_TABLE m
    WHERE m.reporting_supercategory_new = 'Insta Help'
      AND m.country = 'India'
      AND m.BDATE >= '2025-02-01'
      AND m.BDATE < CURRENT_DATE
      AND m.service_delivered = 'true'
    GROUP BY
        DATE_TRUNC('WEEK', m.BDATE),
        m.reporting_city,
        m.provider_id
),
...
ls_summary AS (
    SELECT
        week_start,
        city,
        SUM(late_customer_ids) AS total_late_customer_ids
    FROM ls
    GROUP BY 1, 2
)
...
SELECT
    ...
    ROUND((COALESCE(ls_sum.total_late_customer_ids, 0) * 100.0) / NULLIF(l.SD, 0), 2) AS "Late Show %",
    p.total_providers,
    l.GRs,
    l.SD
FROM l15d_metrics l
...
LEFT JOIN cancellation_data cd 
    ...
    AND l.city = cd.city
LEFT JOIN ls_summary ls_sum
    ON l.week_start = ls_sum.week_start
    AND l.city = ls_sum.city
...
SELECT
    ...
    ROUND((SUM(COALESCE(ls_sum.total_late_customer_ids, 0)) * 100.0) / NULLIF(SUM(l.SD), 0), 2) AS "Late Show %",

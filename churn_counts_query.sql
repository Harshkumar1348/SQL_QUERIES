...
-- FIXED: removed references to undefined aliases n7d, dc, u
do_d7 as (
    SELECT 
        m.*,
        ...
        CASE 
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) = 0
                 THEN '3. D0 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 7
                 THEN '4. D7 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 17
                 THEN '5. D15 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 30
                 THEN '6. D30 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 60
                 THEN '7. D60 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 90
                 THEN '8. D90 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' 
                 AND (CASE WHEN m.ldd IS NULL THEN 0 ELSE (DATE(m.ldd) - DATE(m.app_date)) END) <= 120
                 THEN '9. D120 churn'
            WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Working' THEN '1. Active'
            WHEN m.ldd IS NULL AND DATE(m.app_date) >= CURRENT_DATE - 7 THEN '1. Active'
            ELSE '10. >D120 churn'
...
-- NEW: Aggregate D0 and D7 churn counts by city and ldd_week
churn_agg AS (
    SELECT 
        LOWER(city) AS churn_city,
        DATE(ldd_week) AS churn_week_val,
...
-- NEW: Aggregate D0 and D7 churn counts overall (across all cities) by ldd_week
churn_agg_overall AS (
    SELECT 
        DATE(ldd_week) AS churn_week_val,
        COUNT(DISTINCT CASE WHEN churn_bucket = '3. D0 churn' THEN provider_id END) AS d0_churn_count,
        COUNT(DISTINCT CASE WHEN churn_bucket = '4. D7 churn' THEN provider_id END) AS d7_churn_count
...
-- NEW: D0 and D7 churn counts joined from do_d7 via churn_agg
COALESCE(MAX(ca.d0_churn_count), 0) AS d0_churn_count,
COALESCE(MAX(ca.d7_churn_count), 0) AS d7_churn_count,
...
-- NEW: join churn aggregation by matching city and week
left join churn_agg ca 
    on ca.churn_city = lower(f.city) 
    and ca.churn_week_val = w.week_val
group by 1,2
...
-- NEW: D0 and D7 churn counts joined from do_d7 via churn_agg_overall
COALESCE(MAX(cao.d0_churn_count), 0) AS d0_churn_count,
COALESCE(MAX(cao.d7_churn_count), 0) AS d7_churn_count,
...
from weeks w
left join final f on 1=1
-- NEW: join overall churn aggregation by matching week only
left join churn_agg_overall cao 
    on cao.churn_week_val = w.week_val
group by 1,2

...
-- Calculate L50 rating (average of last 50 bookings per provider)
ranked_ratings AS (
    SELECT
        provider_id,
        rating,
...
-- Calculate weekly average rating (for weeks >= 2026-01-19)
weekly_avg_rating AS (
    SELECT
        provider_id,
        DATE_TRUNC('WEEK', BDATE) AS WEEK,
        ROUND(
            SUM(CASE WHEN service_delivered = 1 AND rating IS NOT NULL THEN rating ELSE 0 END) 
            / NULLIF(COUNT(DISTINCT CASE WHEN service_delivered = 1 AND rating IS NOT NULL THEN customer_request_id END), 0), 
            2
        ) AS weekly_avg
    FROM public.MASTER_DATA_EXPLORE_TABLE
    WHERE DATE_TRUNC('WEEK', BDATE) >= '2026-01-19'
    GROUP BY provider_id, DATE_TRUNC('WEEK', BDATE)
),

-- Original booking_avg_rating CTE (kept for reference if needed)
booking_avg_rating AS (
    SELECT 
        provider_id,
...
base AS (
    SELECT
        ...
        bar.week,
        bar.avg_rating,
        -- Use weekly_avg if week >= 2026-01-19, otherwise use avg_rating_l50
        CASE 
    ...
    LEFT JOIN booking_avg_rating bar 
        ON mm.provider_id = bar.provider_id
    LEFT JOIN avg_rating_last_50 arl50
        ON mm.provider_id = arl50.provider_id
    LEFT JOIN weekly_avg_rating war
        ON mm.provider_id = war.provider_id 
        AND bar.week = war.week
)
...
SELECT
    ...
    ROUND(AVG(avg_l50_rating), 2) AS avg_l50_rating,
    ROUND(

-- Weekly trainer performance view with weeks horizontal (wk_11..wk_0).
-- Adjust the number of weeks in the final SELECT if needed.
...
view2 AS (
    ...
    rating_raw AS (
        SELECT DISTINCT
            ...
            customer_request_id,
            RANK() OVER (
                PARTITION BY responded_pro_booking
                ORDER BY DATE(rating_date) + 3
            ) AS rnk
...
tm_age AS (
    SELECT
        ...
        DATEDIFF('day', MIN(DATE(created_at_ist)), CURRENT_DATE()) AS tmm,
        CASE
            WHEN DATEDIFF('day', MIN(DATE(created_at_ist)), CURRENT_DATE()) > 60 THEN 'LLC Trainer'
            ELSE 'ELC Trainer'
        END AS age_tm
    FROM poco
    GROUP BY created_by
...
weekly_counts AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN tmm <= 60 THEN trainer END) AS trainers_lt_60_days
    FROM trainer_week
    GROUP BY week
),

metric_values AS (
    SELECT 'total_elc_trainers' AS metric, week, total_elc_trainers AS value FROM weekly_counts
    UNION ALL
    SELECT 'trainers_score_gt_75', week, trainers_score_gt_75 FROM weekly_counts
    UNION ALL
    SELECT 'trainers_elc_pip_pct_lt_20', week, trainers_elc_pip_pct_lt_20 FROM weekly_counts
    UNION ALL
    SELECT 'trainers_lt_60_days', week, trainers_lt_60_days FROM weekly_counts
)

SELECT
    metric,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -11, CURRENT_DATE())) THEN value END) AS wk_11,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -10, CURRENT_DATE())) THEN value END) AS wk_10,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -9, CURRENT_DATE())) THEN value END) AS wk_9,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -8, CURRENT_DATE())) THEN value END) AS wk_8,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -7, CURRENT_DATE())) THEN value END) AS wk_7,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -6, CURRENT_DATE())) THEN value END) AS wk_6,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -5, CURRENT_DATE())) THEN value END) AS wk_5,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -4, CURRENT_DATE())) THEN value END) AS wk_4,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -3, CURRENT_DATE())) THEN value END) AS wk_3,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -2, CURRENT_DATE())) THEN value END) AS wk_2,
    MAX(CASE WHEN week = DATE_TRUNC('week', DATEADD('week', -1, CURRENT_DATE())) THEN value END) AS wk_1,
    MAX(CASE WHEN week = DATE_TRUNC('week', CURRENT_DATE()) THEN value END) AS wk_0
FROM metric_values
GROUP BY metric
ORDER BY metric;

-- Provider Trainer Metrics Query with L50 Average Rating
-- Modified to include l50_avg_rating with conditional logic:
-- - Before Jan 19, 2026 week: Rolling 50 rating average
-- - From Jan 19, 2026 week onwards: Weekly average rating
...
-- Rating events: Extract all rating records with week truncation
rating_events AS (
    SELECT 
        provider_id, 
        rating, 
...
-- Calculate rolling L50 average rating (for weeks before Jan 19, 2026)
...
-- Get the last rolling L50 value per provider per week
rolling_l50_per_week AS (
    SELECT 
        provider_id, 
        rating_week, 
        ROUND(rolling_l50, 2) AS avg_rating_l50_rolling
    FROM rating_rolling_calc
    QUALIFY ROW_NUMBER() OVER (PARTITION BY provider_id, rating_week ORDER BY bdate DESC) = 1
),

-- Calculate simple weekly average rating (for weeks from Jan 19, 2026 onwards)
weekly_avg_rating AS (
    SELECT 
        provider_id,
        rating_week,
        ROUND(AVG(rating), 2) AS avg_rating_weekly
    FROM rating_events
    GROUP BY provider_id, rating_week
),

-- Combine both metrics with conditional logic
l50_avg_rating_final AS (
    SELECT 
        ...
        CASE 
            ...
            THEN w.avg_rating_weekly
            ELSE r.avg_rating_l50_rolling
...
-- Get the most recent L50 average rating per provider
latest_l50_rating AS (
    SELECT 
        provider_id,
        l50_avg_rating
    FROM l50_avg_rating_final
    QUALIFY ROW_NUMBER() OVER (PARTITION BY provider_id ORDER BY rating_week DESC) = 1
),

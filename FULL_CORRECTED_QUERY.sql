/*
================================================================================
FULL CORRECTED QUERY WITH FIXED CHURN_STATUS AND CHURN_PROS LOGIC
================================================================================
Changes Applied:
1. Replaced l7d CTE with n7d CTE (Next 7 Days instead of Last 7 Days)
2. Changed working_hrs threshold from >8 to >5
3. Added active_status calculation
4. churn_status now combines active_status AND n7d_status correctly
================================================================================
*/

WITH main AS (
    -- Your existing main CTE here
    SELECT * FROM your_main_table
),

trainer_mapping AS (
    -- Your existing trainer_mapping CTE here
    SELECT provider_id, trainer_name FROM your_trainer_mapping_table
),

current_hub AS (
    -- Your existing current_hub CTE here
    SELECT provider_id, hub_name FROM your_current_hub_table
),

-- ============================================================================
-- CORRECTED: n7d CTE (replaced l7d)
-- Calculates working hours for NEXT 7 days (current_date+1 to current_date+7)
-- ============================================================================
n7d AS (
    SELECT 
        pdf.provider_id,
...
FROM (
    SELECT 
        ...
        COALESCE(n7d.N7D_status, 'Not Working') AS N7D_status,
        ...
        -- Provider is 'churn' ONLY if:
        --   1. active_status = 'Churned' (no delivery in last 7 days, not new)
        --   AND
        --   2. N7D_status = 'Not Working' (no working hours for next 7 days)
        -- ================================================================
        CASE 
            WHEN (
                CASE 
        ...
        -- ================================================================
        -- Updated churn_bucket logic (consistent with corrected logic)
        -- ================================================================
        CASE    
            WHEN COALESCE(n7d.N7D_status, 'Not Working') = 'Working' THEN '1. Active'
        ...
        -- ================================================================
        -- Updated ELC_LLC (consistent with corrected logic)
        -- ================================================================
        CASE 
            WHEN COALESCE(n7d.N7D_status, 'Not Working') = 'Working' THEN '1. Active'
            WHEN m.ldd > CURRENT_DATE - 7 THEN '1. Active'
    ...
    FROM main m
    -- ================================================================
    -- CORRECTED: Changed from l7d to n7d
    -- ================================================================
...
The churn_pros count is calculated as:
  count(distinct case when life_cycle='ELC' and churn_status='churn' then provider_id end) as churn_pros

With the CORRECTED churn_status logic, this will now correctly count only providers who:
1. Are in ELC (Early Life Cycle, age <= 30 days)
2. Have NOT delivered in the last 7 days (or are not newly approved)
3. AND have NO working hours marked for the next 7 days

Previously, the query was incorrectly using L7D (last 7 days working hours) which meant:
- A provider who had worked LAST week but has NO future work scheduled would NOT be counted as churned
- This caused churn_pros to be undercounted

Now with N7D (next 7 days working hours):
- A provider who has not delivered recently AND has no future work scheduled IS correctly counted as churned
- This gives accurate churn_pros count

KEY DIFFERENCES SUMMARY:
-----------------------
| Aspect          | OLD (Incorrect)              | NEW (Correct)                    |
|-----------------|------------------------------|----------------------------------|
| Date Range      | CURRENT_DATE - 6 to TODAY    | CURRENT_DATE + 1 to +7           |
| Looking at      | PAST working hours           | FUTURE working hours             |
| Threshold       | > 8 hours                    | > 5 hours                        |
| churn_status    | Based only on L7D_status     | Based on active_status AND N7D   |
================================================================================
*/

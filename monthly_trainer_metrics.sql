...
provider_name,
DATE_TRUNC('MONTH',LAST_DELIVERY_DATE) AS ldd_month,
city,
avg_rating,
date(approval_date)as app_date,
date_trunc('month',app_date) as approval_month,
...
a.PROVIDER_ID, date_trunc('month',date(a.date)) as cm_month,
date,
...
Rating_raw AS (
    SELECT 
        ...
        date_trunc('month',ratings_date) AS rating_month,
        customer_request_id,
...
,
l10rating as (select distinct provider_id,rating_month,
...
rr as (select distinct provider_id,rating_month,
...
from rating_raw r 
left join rr on r.provider_id=rr.provider_id and r.rating_month=rr.rating_month
group by 1,2,3)
,
pp as (SELECT 
        responded_pro_booking AS provider_id, 
        DATE_TRUNC('MONTH', date(paf.cancelled_at)) AS paf_month,
...
from paf_raw pr left join 
   pp on pr.provider_id=pp.provider_id and pp.paf_month=pr.paf_month
)
,
final_raw as 
(select b.*,
case when active_status='Churn'  then 'churn' else 'active' end as churn_status,
cm_month,cm_month-app_date as age,
total_rated_jobs,
bad_rated_jobs,
request_ids,
avg_weekly_rating,
...
left join rating r on r.provider_id=cm.provider_id and r.rating_month=cm.cm_month
left join l10rating r10 on r10.provider_id=cm.provider_id and r10.rating_month=cm.cm_month
left join paf p on p.provider_id=cm.provider_id and p.paf_month=cm.cm_month
...
approval_month,
cm_month,
rank() over(partition by provider_id order by cm_month) as month_num,
age,
gr,
sd,
l15_sd,
...
date_trunc('month',date(bdate_final)) as month,
...
COUNT(DISTINCT CASE
    WHEN net_status = 'net_request'
     AND service_delivered = true
     AND bdate BETWEEN
         DATEADD(day, -14, LAST_DAY(date(bdate_final)))
         AND LAST_DAY(date(bdate_final))
...
group by 1,2
) u on  u.responded_pro_booking=a.provider_id and u.month=a.cm_month
--where provider_id='62c9b1b2c4c415002866407b'
order by provider_id,cm_month)
,
manual_mapping AS (
    SELECT * FROM VALUES 
...
('685e57235c65910023a021c8','Neeraj'),
('687492789f5dce0024f17857','Manisharathod'),
('6870fb373df681002419fbb1','Jayashree'),
...
dnps_calc_base AS (
    SELECT 
        ...
        r.PROVIDER_ID,
        r.service_delivered,
        DATE_TRUNC('month', r.BDATE) as month,
        CASE 
...
avg_rating_l15 AS (
    SELECT
        provider_id,
        MONTH as month,
        sum(rating) as l15_sum_rating,
         NULLIF(COUNT(rating), 0) as rated_l15_count
    FROM ranked_ratings
    WHERE rating_rank <= 15
    GROUP BY provider_id, month
),
...
left join dnps_calc dnps on f.provider_id = dnps.provider_id AND f.cm_month = dnps.perf_month  
-- Modified join to be only on provider_id
left join avg_rating_l15 arl on f.provider_id = arl.provider_id AND f.cm_month = arl.month
),
...
perf_month as "perf_month::multi-filter",
...
count(distinct case when life_cycle='ELC' then provider_id end) as hh_eligible_pro

/*
  Trainer D0/D7 Churn Query
  -------------------------
  Combines trainer scoring metrics (view1) with provider-level churn classification
  using the l7d calendar-marking + ranked active-profile approach, then aggregates
  D0 and D7 churn counts per trainer per week.

  Key changes from original:
  1. Added l7d CTE (last-7-day calendar marking) and ranked CTE (active profile block status)
  2. Replaced the simple active_status-based churn with the combined l7d + ranked churn logic
  3. Added churn_bucket classification (D0, D7, D15, D30, D60, D90, D120, >D120)
  4. Final output includes d0_churn_count and d7_churn_count as separate columns per trainer/week
*/
...
/* ======================================================================
   view2 — provider-level weekly performance WITH improved churn logic
   ====================================================================== */
view2 AS (
  WITH base AS (
      SELECT DISTINCT
          ...
          DATEDIFF('day', DATE(approval_date), CURRENT_DATE) AS provider_age,
          CASE
      ...
      LEFT JOIN (
          SELECT DISTINCT
              provider_id,
              date,
              COUNT(CASE WHEN status IN ('unmarked','marked leave','marked working') THEN provider_id END) AS total_hours,
              COUNT(CASE WHEN status IN ('marked working') THEN provider_id END)                           AS marked_working
          ...
          ON acm.provider_id = pdf.provider_id
      WHERE customer_category_key = 'insta_maids'
  ...
  /* ---- L7D calendar-marking churn signal ---- */
  l7d AS (
      SELECT
          pdf.provider_id,
          CASE
  ...
  /* ---- Ranked active-profile churn signal ---- */
  ranked AS (
      SELECT
          provider_id,
          reason,
  ...
  cm AS (
      SELECT DISTINCT
          ...
          COUNT(CASE WHEN status IN ('marked working') THEN a.provider_id END) AS marked_working,
          COUNT(CASE WHEN status IN ('marked leave')   THEN a.provider_id END) AS marked_leave
  ...
  cm_final AS (
      SELECT DISTINCT
          cm.*,
          COALESCE(avg_working_hours, 0) AS avg_wrkn_hrs
      FROM cm
  ...
  rr AS (
      SELECT DISTINCT
          provider_id,
          rating_week,
          LISTAGG(DISTINCT customer_request_id, ', ')
              WITHIN GROUP (ORDER BY customer_request_id ASC) AS request_ids
      FROM rating_raw
      WHERE rating < 4.5
      GROUP BY 1, 2
  ),
  ...
  pp AS (
      SELECT
          ...
          LISTAGG(DISTINCT paf.customer_request_id, ', ')
              WITHIN GROUP (ORDER BY paf.customer_request_id ASC) AS paf_request_ids
  ...
  final_raw AS (
      SELECT
          b.*,
          cm_week,
          DATEDIFF('day', b.app_date, cm_week) AS age,
          total_rated_jobs,
          bad_rated_jobs,
          request_ids,
          avg_weekly_rating,
  ...
  /* Step 1: compute churn_status by joining l7d + ranked */
  final_with_churn_status AS (
      SELECT
          a.provider_id,
          a.provider_name,
          a.city,
          ...
          a.provider_age,
          a.cm_week,
          RANK() OVER (PARTITION BY a.provider_id ORDER BY a.cm_week) AS week_num,
          a.age,
          u.gr,
          u.sd,
          a.total_rated_jobs,
          a.bad_rated_jobs,
          a.request_ids,
          a.avg_rating AS overall_avg_rating,
          a.f10_sum_weekly_rating,
          a.f10_sum_weekly_rated_jobs,
          a.avg_weekly_rating,
          a.total_rating_sum,
          a.paf_count,
          a.paf_request_ids,
          a.total_weekday_leaves + (a.total_weekend_leaves * 2) AS total_graded_leaves,
          CASE
              ...
              THEN '1. Active'
              WHEN a.ldd IS NULL AND a.app_date >= CURRENT_DATE - 7
              THEN '1. Active'
          END AS churn_status
      FROM final_raw a
      LEFT JOIN (
      ...
      LEFT JOIN l7d     ON a.provider_id = l7d.provider_id
      LEFT JOIN ranked ran ON a.provider_id = ran.provider_id AND ran.rn = 1
  ),

  /* Step 2: derive churn_bucket from the already-computed churn_status */
  final AS (
      SELECT
          *,
          CASE
              WHEN churn_status = '1. Active'                          THEN '1. Active'
              WHEN provider_age = 0  AND churn_status = '3. Churn'     THEN '3. D0 churn'
              WHEN provider_age <= 7  AND churn_status = '3. Churn'    THEN '4. D7 churn'
              WHEN provider_age <= 17 AND churn_status = '3. Churn'    THEN '5. D15 churn'
              WHEN provider_age <= 30 AND churn_status = '3. Churn'    THEN '6. D30 churn'
              WHEN provider_age <= 60 AND churn_status = '3. Churn'    THEN '7. D60 churn'
              WHEN provider_age <= 90 AND churn_status = '3. Churn'    THEN '8. D90 churn'
              WHEN provider_age <= 120 AND churn_status = '3. Churn'   THEN '9. D120 churn'
              ELSE '10. >D120 churn'
          END AS churn_bucket
      FROM final_with_churn_status
  ),
  ...
  lst AS (
      SELECT DISTINCT
          ...
          COALESCE(bad_rated_jobs, 0)       AS bad_rated_jobs,
          request_ids,
          COALESCE(paf_count, 0)            AS paf_count,
          paf_request_ids,
          COALESCE(total_graded_leaves, 0)  AS total_graded_leaves
      FROM final f
      LEFT JOIN (
          SELECT * FROM trainer_mapping WHERE trainer_name IS NOT NULL
      ) tm ON f.provider_id = tm.provider_id
  ),
...
/* ==================================================================
   FINAL OUTPUT — trainer metrics + D0 / D7 churn counts per week
   ================================================================== */
SELECT DISTINCT

-- Trainer Weekly Metrics Query with Conditional L50 Rating Logic
-- Logic: 
--   Before Jan 19, 2026: Traditional L50 (average of last 50 ratings per provider)
--   From Jan 19, 2026: Weekly average rating (sum(rating) / count(distinct customer_request_id where service_delivered=1 and rating is not null))
...
-- PAF and Leaves data (latest record per provider)
paf_leaves AS (
    SELECT
        provider_id,
        paf,
...
-- Late show / Working status check
l7d AS (
    SELECT
        provider_id,
        CASE
...
-- ============================================================================
-- L50 RATING CALCULATION - CONDITIONAL LOGIC
-- ============================================================================

-- Method 1: Traditional L50 Rating (for weeks BEFORE Jan 19, 2026)
-- Ranks all ratings by bdate DESC and takes average of last 50
ranked_ratings AS (
    SELECT
        provider_id,
        rating,
...
-- Traditional L50: Average of last 50 ratings per provider per week
avg_rating_last_50_traditional AS (
    SELECT
        provider_id,
        week,
...
-- Method 2: Weekly Average Rating (for weeks FROM Jan 19, 2026 onwards)
-- sum(rating) / count(distinct customer_request_id where service_delivered=1 and rating is not null)
weekly_avg_rating_new AS (
    SELECT
        provider_id,
        ...
        2) AS avg_rating_l50
    FROM public.MASTER_DATA_EXPLORE_TABLE
    WHERE DATE(BDATE) >= '2026-01-19'
    GROUP BY provider_id, DATE_TRUNC('WEEK', DATE(BDATE))
),

-- Combine both methods based on date threshold
combined_avg_rating AS (
    -- Traditional L50 for weeks before Jan 19, 2026
    SELECT 
        provider_id,
        week,
        avg_rating_l50
    FROM avg_rating_last_50_traditional
    WHERE week < DATE_TRUNC('WEEK', DATE('2026-01-19'))
    
    UNION ALL
    
    -- New weekly average for weeks from Jan 19, 2026 onwards
    SELECT 
        provider_id,
        week,
        avg_rating_l50
    FROM weekly_avg_rating_new
),

-- ============================================================================
-- BASE TABLE - Joining all components
-- ============================================================================
base AS (
    SELECT
        mm.trainer_name,
    ...
    LEFT JOIN combined_avg_rating car 
        ON mm.provider_id = car.provider_id
)
...
SELECT
    ...
    ROUND(AVG(avg_rating_l50), 2) AS avg_l50_rating,
    ROUND(

/*
  FIX: Added "verification_blocked" CTE from PROVIDERXDATE__ACTIVE_PROFILE__HOURLY__FACTS
  to catch providers blocked with reason "verification failed" within 15 days of approval_date.

  The existing NOT EXISTS on provider_master.temporary_block_reasons is kept as a first layer.
  The new NOT EXISTS on PROVIDERXDATE__ACTIVE_PROFILE__HOURLY__FACTS acts as a second safety net.
*/

WITH verification_blocked AS (
    SELECT
        provider_id,
        reason,
        temporary_block,
        DATE(temp_block_since_date) AS block_date
    FROM PROVIDERXDATE__ACTIVE_PROFILE__HOURLY__FACTS
    WHERE customer_category_key = 'insta_maids'
      AND CUSTOMER_CATEGORY_KEY != 'home_cooks'
      AND REGEXP_LIKE(LOWER(reason), 'verification\\s*failed')
...
WHERE a.customer_category_key = 'insta_maids'
  ...
  /* ── Layer 1: existing check against provider_master JSON ── */
  AND NOT EXISTS (
      SELECT 1
      FROM public.provider_master p,
           LATERAL FLATTEN(
             input => COALESCE(
               TRY_PARSE_JSON(p.temporary_block_reasons),
               PARSE_JSON('[]')
             )
           ) t
      WHERE p.provider_id = a.provider_id
        AND REGEXP_LIKE(LOWER(COALESCE(t.value:reason::STRING,'')), 'verification\\s*failed')
        AND COALESCE(
              TRY_TO_TIMESTAMP_NTZ(t.value:created_at::STRING),
              TRY_TO_TIMESTAMP_NTZ(t.value:createdAt::STRING),
              IFF(
                REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{13}') IS NOT NULL,
                DATEADD(MILLISECOND,
                        TRY_TO_NUMBER(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{13}')),
                        '1970-01-01'::TIMESTAMP_NTZ),
                NULL
              ),
              IFF(
                REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{10}') IS NOT NULL,
                DATEADD(SECOND,
                        TRY_TO_NUMBER(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{10}')),
                        '1970-01-01'::TIMESTAMP_NTZ),
                NULL
              )
            ) >= b.approval_date::TIMESTAMP_NTZ
        AND COALESCE(
              TRY_TO_TIMESTAMP_NTZ(t.value:created_at::STRING),
              TRY_TO_TIMESTAMP_NTZ(t.value:createdAt::STRING),
              IFF(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{13}') IS NOT NULL,
                  DATEADD(MILLISECOND, TRY_TO_NUMBER(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{13}')), '1970-01-01'::TIMESTAMP_NTZ), NULL),
              IFF(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{10}') IS NOT NULL,
                  DATEADD(SECOND, TRY_TO_NUMBER(REGEXP_SUBSTR(COALESCE(t.value:created_at::STRING,t.value:createdAt::STRING), '\\d{10}')), '1970-01-01'::TIMESTAMP_NTZ), NULL)
            ) < DATEADD(DAY, 46, b.approval_date::TIMESTAMP_NTZ)
  )

  /* ── Layer 2 (NEW): check against PROVIDERXDATE__ACTIVE_PROFILE__HOURLY__FACTS ── */
  AND NOT EXISTS (
      SELECT 1
      FROM verification_blocked vb
      WHERE vb.provider_id = a.provider_id
        AND vb.block_date >= DATE(b.approval_date)
        AND vb.block_date <= DATEADD(DAY, 15, DATE(b.approval_date))
  )

/*
================================================================================
COMPLETE CORRECTED QUERY - CHURN_STATUS AND CHURN_PROS LOGIC FIXED
================================================================================
*/

WITH main AS (
    -- Your existing main CTE - replace with your actual main CTE definition
    SELECT 
        provider_id,
        provider_name,
        city,
        app_date,
        ldd,  -- last_delivery_date
        ldd_week,
        age
    FROM your_main_source_table
),

trainer_mapping AS (
    -- Your existing trainer_mapping CTE
    SELECT provider_id, trainer_name 
    FROM your_trainer_mapping_table
),

current_hub AS (
    -- Your existing current_hub CTE
    SELECT provider_id, hub_name 
    FROM your_current_hub_table
),

-- ============================================================================
-- CORRECTED: n7d CTE (replaces l7d)
-- Now looks at NEXT 7 days (current_date+1 to current_date+7) instead of past
-- Changed threshold from >8 to >5
-- ============================================================================
n7d AS (
    SELECT 
        pdf.provider_id,

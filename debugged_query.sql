WITH v1_base AS (
    ...
    WHERE a.customer_category_key = 'insta_maids'
      ...
      AND DATE_TRUNC('week', DATE(walked_in)) > '2025-05-01'
),
...
v2_base as (
    select distinct pdf.provider_id,
    provider_name,
    city,
    avg_rating,
...
v2_cm as (
    ...
    join v2_base b on b.provider_id=a.provider_id
    where a.provider_id in (select distinct provider_id from v2_base)
    AND START_HOUR_LOCAL BETWEEN 8 AND 19
...
v2_cm_final as (
    ...
    from v2_cm cm left join v2_f_cm f_cm on cm.provider_id=f_cm.provider_id
),
...
v2_rating_raw AS (
    ...
    WHERE customer_category_key = 'insta_maids'
    and provider_id in (select distinct provider_id from v2_base)
      AND rating IS NOT NULL
      --and rating <=4.5
),

v2_l10rating as (
    select distinct provider_id,rating_week,
...
v2_rr as (
    ...
    from v2_rating_raw where rating<4.5
    group by 1,2
),
...
v2_rating as (
    ...
    from v2_rating_raw r 
    left join v2_rr rr on r.provider_id=rr.provider_id and r.rating_week=rr.rating_week
    group by 1,2,3
),
...
v2_paf as (
    ...
    from v2_paf_raw pr left join 
       v2_pp pp on pr.provider_id=pp.provider_id and pp.paf_week=pr.paf_week
),
...
v2_final_raw as (
    ...
    from v2_base b left join v2_cm_final cm on b.provider_id=cm.provider_id
    left join v2_rating r on r.provider_id=cm.provider_id and r.rating_week=cm.cm_week
    left join v2_l10rating r10 on r10.provider_id=cm.provider_id and r10.rating_week=cm.cm_week
    left join v2_paf p on p.provider_id=cm.provider_id and p.paf_week=cm.cm_week
...
v2_lst as (
    ...
    from
    v2_final f left join 
    (select * from v2_trainer_mapping where trainer_name is not null) tm on f.provider_id=tm.provider_id
),
...
final as(SELECT DISTINCT coalesce(v.trainer,v1.trainer_name) as trainer,
    ...
    COALESCE((0.6  * (100 - v1.bad_average_perc)),0),
    2
) AS score
      FROM view1 v

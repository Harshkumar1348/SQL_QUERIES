function syncHandholdingData() {
  ...
  // Now pulling 21 columns (B to V) from Query Output - added Late Show column
  ...
  queryData.forEach((queryRow) => {
    ...
    if (dumpKeyMap.has(key)) {
      ...
      existingRow[20] = queryRow[20]; // Late Show (V) - NEW COLUMN
      ...
      newRow[21] = today; // Column W (Date Added) - shifted from V to W

/**
 * Syncs handholding data from Query Output sheet to Data Dump sheet.
 * 
 * Query Output columns (B to V - 21 columns):
 *   B: Provider ID
 *   C-F: Various data columns
 *   G: Performance Week
 *   H-O: Various data columns
 *   P: Rating Errors
 *   Q: PAF Errors
 *   R: Leaves
 *   S: Week Status
 ...
 *   V: Late Show (NEW)
 * 
 * Data Dump columns (B to X - 23 columns):
 *   B-V: Same as Query Output (21 columns)
 *   W: Date Added
 *   X: Reserved/Padding
 */
function syncHandholdingData() {
  ...
  // Now pulling 21 columns (B to V) from Query Output - includes new Late Show column
  ...
  queryData.forEach((queryRow) => {
    ...
    if (dumpKeyMap.has(key)) {
      ...
      existingRow[20] = queryRow[20]; // Late Show (V) - NEW
      ...
      newRow[21] = today; // Column W (Date Added) - shifted from index 20 to 21

-- Trainer Metrics Query with L50 Average Rating
-- l50_avg_rating logic:
--   - For weeks BEFORE 19th Jan 2025: Use rolling L50 (avg of last 50 ratings)
--   - For weeks FROM 19th Jan 2025 onwards: Use weekly average rating
...
-- =====================================================
-- L50 Average Rating Calculation CTEs
-- =====================================================

-- Step 1: Get all rating events with week information
rating_events AS (
    SELECT 
        provider_id, 
        rating, 
        bdate, 
        DATE_TRUNC('WEEK', DATE(bdate)) AS rating_week
...
-- Step 2: Calculate rolling L50 (last 50 ratings average)
rating_rolling_calc AS (
    SELECT 
        provider_id, 
        rating_week, 
        bdate,
...
-- Step 3: Calculate weekly average rating for each provider per week
weekly_avg_rating AS (
    SELECT 
        provider_id,
        rating_week,
        AVG(rating) AS weekly_avg
    FROM rating_events
    GROUP BY provider_id, rating_week
),

-- Step 4: Get the rolling L50 as of the end of each week (last record per week)
rolling_l50_per_week AS (
    SELECT 
        provider_id, 
        rating_week, 
        ROUND(rolling_l50, 2) AS rolling_l50
    FROM rating_rolling_calc
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY provider_id, rating_week 
        ORDER BY bdate DESC
    ) = 1
),

-- Step 5: Combine both calculations and apply conditional logic
-- For weeks >= 19th Jan 2025: Use weekly average
-- For weeks < 19th Jan 2025: Use rolling L50
avg_rating_l50_final AS (
    SELECT 
        COALESCE(r.provider_id, w.provider_id) AS provider_id,
        COALESCE(r.rating_week, w.rating_week) AS rating_week,
        CASE 
            -- From 19th Jan 2025 week onwards: use weekly average rating
            WHEN COALESCE(r.rating_week, w.rating_week) >= DATE_TRUNC('WEEK', DATE('2025-01-19'))
                THEN ROUND(w.weekly_avg, 2)
            -- Before 19th Jan 2025: use rolling L50
            ELSE r.rolling_l50
        END AS l50_avg_rating
    FROM rolling_l50_per_week r
    FULL OUTER JOIN weekly_avg_rating w
        ON r.provider_id = w.provider_id 
        AND r.rating_week = w.rating_week
),

-- Step 6: Get the most recent l50_avg_rating for each provider
-- (to use as the current/latest rating in the final report)
latest_l50_rating AS (
    SELECT 
        provider_id,
        l50_avg_rating
    FROM avg_rating_l50_final
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY provider_id 
        ORDER BY rating_week DESC
    ) = 1
),

-- =====================================================
-- Base query combining all metrics
-- =====================================================
base AS (
    SELECT
        mm.trainer_name,
    ...
    LEFT JOIN latest_l50_rating lr
        ON mm.provider_id = lr.provider_id
)

SELECT
    trainer_name AS trainer,

    COUNT(DISTINCT provider_id)  AS total_px,
    ...
    -- New column: Average L50 rating across all providers per trainer
    ROUND(AVG(l50_avg_rating), 2) AS avg_l50_rating

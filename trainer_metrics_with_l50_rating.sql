...
-- Logic: For weeks before 19th Jan 2025, use rolling L50 average
--        For weeks from 19th Jan 2025 onwards, use simple weekly average
...
-- Rating events: base data for rating calculations
rating_events AS (
    SELECT 
        provider_id, 
        rating, 
...
-- Rolling L50 calculation (for weeks BEFORE 19th Jan 2025)
...
-- L50 average rating for weeks BEFORE 19th Jan 2025
avg_rating_l50_before_jan19 AS (
    SELECT 
        provider_id, 
        rating_week AS week, 
        ROUND(rolling_l50, 2) AS avg_rating_l50
    FROM rating_rolling_calc
    WHERE rating_week < DATE('2025-01-19')  -- Before 19th Jan week
...
-- Simple weekly average rating for weeks FROM 19th Jan 2025 onwards
avg_rating_weekly_from_jan19 AS (
    SELECT 
        provider_id,
        rating_week AS week,
        ROUND(AVG(rating), 2) AS avg_rating_l50
    FROM rating_events
    WHERE rating_week >= DATE('2025-01-19')  -- From 19th Jan week onwards
    GROUP BY provider_id, rating_week
),

-- Combined L50 rating: Union of both calculations
l50_rating_combined AS (
    SELECT provider_id, week, avg_rating_l50
    FROM avg_rating_l50_before_jan19
    
    UNION ALL
    
    SELECT provider_id, week, avg_rating_l50
    FROM avg_rating_weekly_from_jan19
),

-- Get the most recent l50 rating per provider (to use in final output)
latest_l50_rating AS (
    SELECT 
        provider_id,
        avg_rating_l50 AS l50_avg_rating
    FROM l50_rating_combined
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY provider_id 
        ORDER BY week DESC
    ) = 1
...
SELECT
    ...
    -- New: L50 Average Rating (simple weekly avg from 19th Jan, rolling L50 before)
    ROUND(AVG(l50_avg_rating), 2) AS l50_avg_rating

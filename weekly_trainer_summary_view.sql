/*
================================================================================
  WEEKLY TRAINER SUMMARY VIEW
  -----------------------------------------------------------------------
  Goal  : Weeks displayed HORIZONTALLY (as columns).
          Four metrics shown per week:
            1) Total trainers who are in ELC that week
            2) Trainers with Score > 75
            3) Trainers with ELC PIP PCT < 20
            4) Trainers with less than 60 days experience (ELC Trainers)

  Two versions provided:
    VERSION 1 – Weekly summary (weeks as rows, metrics as columns)
                Best for BI tools that can pivot natively (Metabase, Looker, etc.)
    VERSION 2 – Fully pivoted view (weeks as column headers, metrics as rows)
                Uses Snowflake UNPIVOT + PIVOT.
================================================================================
*/

-- ============================================================================
--  VERSION 1 :  WEEKLY SUMMARY  (weeks as rows – BI-tool friendly)
-- ============================================================================
...
-- ---------------------------------------------------------------
--  "final" CTE : merge view1 (scores) + view2 (operational KPIs)
-- ---------------------------------------------------------------
final AS (
    SELECT DISTINCT
...
-- ---------------------------------------------------------------
--  trainer_weekly_detail :  per-trainer, per-week row with all
--                           metrics & flags needed for aggregation
-- ---------------------------------------------------------------
trainer_weekly_detail AS (
    SELECT
        ...
        a.score,
        CASE WHEN a.elc_pip_pct < 20  THEN 'Less Than 20'    ELSE 'Greater than 20' END AS elc_pip_less_20,
        CASE WHEN a.score > 75        THEN 'Greater than 75'  ELSE 'Less than 75'    END AS score_range,
        DENSE_RANK() OVER (PARTITION BY a.week ORDER BY a.score DESC) AS final_rank
    FROM final a
...
weekly_summary AS (
    SELECT
        week,
        -- 1) Total trainers who are in ELC that week (have ELC partners assigned)
...
-- ===================================================================
--  VERSION 1 OUTPUT:  Weekly summary – weeks as rows, metrics as cols
--  (Use this if your BI tool handles pivoting natively)
-- ===================================================================
SELECT
    week,

...
do_d7 as (SELECT 
    ...
    COALESCE(u.last_7_days_util, 0) AS last_7_days_util,
      CASE   WHEN COALESCE(l7d.L7D_status, 'Unknown') = 'Not Working' THEN '3. Churn'
...
,final as 
(
select b.city, b.provider_id, provider_name, wi_week, sa_week,sf_week, sp_week, ts_week, stt_week, it_week, tp_week, approval_week, pr_week,
CASE 
...
count(distinct case when f.wi_week is not null and f.wi_week = w.week_val then f.provider_id end ) as wi,
count(distinct case 
        when f.sp_week = w.week_val 
          OR f.sf_week = w.week_val
        then f.provider_id 
end ) as sa,
count(distinct case when f.sp_week = w.week_val then f.provider_id end ) as sp,
count(distinct case when f.pr_week = w.week_val then f.provider_id end ) as pr,
count(distinct case when f.it_week = w.week_val then f.provider_id end ) as it,
count(distinct case when f.it_week = w.week_val and tp_week is not null then f.provider_id end )as tp,
count(distinct case when f.approval_week = w.week_val then f.provider_id end ) as app,

-- D0 and D7 Churn Counts
...
count(distinct case when sp_week = w.week_val then f.provider_id end )as sp,
count(distinct case when pr_week = w.week_val then f.provider_id end )as pr,
count(distinct case when it_week = w.week_val then f.provider_id end )as it,
count(distinct case when it_week = w.week_val and tp_week is not null then f.provider_id end )as tp,
count(distinct case when approval_week = w.week_val then f.provider_id end ) as app,

-- D0 and D7 Churn Counts for Overall
count(distinct case when dd.churn_bucket = '3. D0 churn' and dd.ldd_week = w.week_val then dd.provider_id end) as d0_churn_count,
count(distinct case when dd.churn_bucket = '4. D7 churn' and dd.ldd_week = w.week_val then dd.provider_id end) as d7_churn_count,

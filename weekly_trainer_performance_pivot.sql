/*
  ...
  ==========================================
  Weeks are displayed HORIZONTALLY (as columns).
  Metrics (rows):
    1. TOTAL_TRAINERS_IN_ELC   – trainers who have ELC partners that week
    2. TRAINERS_SCORE_GT_75    – trainers whose weighted score > 75
    3. TRAINERS_ELC_PIP_LT_20  – trainers whose ELC PIP % < 20
    4. TRAINERS_LT_60_DAYS     – trainers with < 60 days tenure (ELC Trainer)
*/
...
view2 AS (
  WITH base AS (
      SELECT DISTINCT
          ...
          SUM(CASE WHEN acm.date BETWEEN CURRENT_DATE + 1 AND CURRENT_DATE + 7
                   THEN marked_working END)  AS working_hrs,
          CASE WHEN working_hrs > 5 THEN 'Working' ELSE 'Not Working' END AS N7D_status
      FROM provider__daily__facts pdf
      LEFT JOIN (
          SELECT DISTINCT
              provider_id, date,
              COUNT(CASE WHEN status IN ('unmarked','marked leave','marked working')
                         THEN provider_id ELSE NULL END) AS total_hours,
              COUNT(CASE WHEN status IN ('marked working')
                         THEN provider_id ELSE NULL END) AS marked_working
  ...
  PAF_raw AS (
      SELECT
          responded_pro_booking AS provider_id,
          DATE_TRUNC('week', DATE(paf.cancelled_at)) AS paf_week,
          COUNT(DISTINCT CASE WHEN paf.ns_provider_at_fault_flag = 1
                              THEN paf.customer_request_id END) AS paf_count
      FROM REQUEST__BCE__HOURLY__FACTS paf
      JOIN request__hourly__facts rhf ON rhf.customer_request_id = paf.customer_request_id
      WHERE paf.customer_category_key = 'insta_maids'
        AND provider_id IN (SELECT DISTINCT provider_id FROM base)
  ...
  final AS (
      ...
      LEFT JOIN (
          SELECT DISTINCT
              ...
              COUNT(DISTINCT CASE WHEN net_status = 'net_request' AND service_delivered = 'true'
                                  THEN customer_request_id END) AS sd
          FROM master_data
          WHERE customer_category_key = 'insta_maids'
  ...
  SELECT
      ...
      COUNT(DISTINCT CASE WHEN life_cycle = 'ELC' AND churn_status = 'churn'
...
-- =====================================================================
-- "final" CTE: merge view1 (scores) + view2 (ELC metrics)
-- =====================================================================
final AS (
    SELECT DISTINCT
...
-- =====================================================================
-- Trainer-age classification (< 60 days = ELC Trainer)
-- =====================================================================
POCO AS (
    SELECT
        provider_id,
...
-- =====================================================================
-- trainer_weekly: one row per trainer × week with all needed columns
-- (replaces the old final SELECT)
-- =====================================================================
trainer_weekly AS (
    SELECT
        a.trainer,
...
-- =====================================================================
-- weekly_summary: aggregate the four requested metrics per week
-- =====================================================================
weekly_summary AS (
    SELECT
        week,
        -- 1. Total trainers who are in ELC that week (i.e. have ELC partners)
        COUNT(DISTINCT trainer)
            AS TOTAL_TRAINERS_IN_ELC,

        -- 2. Trainers with score > 75
        COUNT(DISTINCT CASE WHEN score > 75 THEN trainer END)
            AS TRAINERS_SCORE_GT_75,

        -- 3. Trainers with ELC PIP % < 20
        COUNT(DISTINCT CASE WHEN elc_pip_pct < 20 THEN trainer END)
            AS TRAINERS_ELC_PIP_LT_20,

        -- 4. Trainers with < 60 days tenure (ELC Trainer)
        COUNT(DISTINCT CASE WHEN age_tm = 'ELC Trainer' THEN trainer END)
            AS TRAINERS_LT_60_DAYS
    FROM trainer_weekly
    GROUP BY week
),

-- =====================================================================
-- Unpivot the four metrics so we get (week, metric_name, metric_value)
-- =====================================================================
unpivoted AS (
    SELECT week, metric_name, metric_value
    FROM weekly_summary
    UNPIVOT (
        metric_value FOR metric_name IN (
            TOTAL_TRAINERS_IN_ELC,
            TRAINERS_SCORE_GT_75,
            TRAINERS_ELC_PIP_LT_20,
            TRAINERS_LT_60_DAYS
        )
    )
)
...
-- =====================================================================
-- Final pivot: weeks become columns, metrics become rows
-- Uses Snowflake dynamic PIVOT with IN (ANY)
-- =====================================================================
SELECT *
FROM unpivoted
PIVOT (
    MAX(metric_value)
    FOR week IN (ANY ORDER BY week)
)
